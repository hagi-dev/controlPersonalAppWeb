...
const Contrato = () => {
  ...
  const columns = [
    ...
    { title: "DNI", field: "dni", align: "left", filtering: true },
    { title: "Personal", field: "nombre" },
    { title: "Puesto", field: "cargo", align: "left" },
    {
      title: "Horario",
      field: "horario",
      width: "250px",
    },
    {
      title: "Incio",
      field: "fecha_inn",
      render: (rowData) => <p>{formatearFechaEspañol(rowData.fecha_inn)}</p>,
    },
    {
      title: "Finaliza",
      field: "fecha_out",
      render: (rowData) => <p>{formatearFechaEspañol(rowData.fecha_out)}</p>,
    },
  ...
  const enviarPut2 = async () => {
    ...
    cambiarEstadoModal3(!estadoModal3);
    await axios
      .put(`/contrato/update/${getData.id}`, getData)
      .then((res) => {
        setRespuesta(res.data), console.log("esta es la respuesta", res);
      })
      .catch((err) => {
        console.log(err);
      });
    ...
    <div className="Contrato">
                ...
                actions={[
                  ...
                  },
                  {
                    ...
                    onClick: async (event, rowData) => {
                      window.location.href = `/registro-contratos/${rowData.contratoId}`;
                  ...
                  },
                  {
                    ...
                    onClick: async (event, rowData) => {
                        if (window.confirm("¿Estás seguro de eliminar este contrato?")) {
                            await axios.delete(`/contrato/${rowData.contratoId}`);
                            getContract();
                        }
                    },
                  },
                  {
                    ...
                    onClick: (event, rowData) =>
                      (window.location.href = `/registro-contratos/nuevo`),
                    isFreeAction: true,
                  },
                  {
                    icon: () => (
      ...
      >
        ...
        >
          <button
            type="button"
            onClick={() => deleteCerrar(idseleccionado)}
            className="button"
            style={{ width: "30%", height: "30px" }}
          >
          ...
          <button
            type="button"
            onClick={() => deleteEliminar(idseleccionado)}
            className="button"
            style={{ width: "30%", height: "30px" }}
          >
            <h5>eliminar contrato</h5>
          </button>
          <button
            type="button"
            className="button"
