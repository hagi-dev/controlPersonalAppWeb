...
import "date-fns";
import { display } from '../../node_modules/@material-ui/system/es';
import {
  KeyboardDatePicker,
...
const RegistrarContrato = () => {
  ...
  const enviarPost = async () => {
    await setGetData((prevState) => ({
      ...prevState,
      idTipoTrabajador: cargo,
    }));
  ...
  const enviarPost2 = async () => {
    ...
    await axios
      .post("/contrato/registrar", getData)
      .then((res) => {
  ...
  const verId = async (e) => {
    await axios
      .post("/personal/ids", getData)
      .then((res) => {
        console.log("re res res",res);
        if (res.data.ok) {
    ...
    await axios
      .get(`/contrato/validate/fecha/${getData.dni}`)
      .then((res) => {
    ...
    await axios
      .get(`/contrato/${getData.dni}`)
      .then((res) => {
        console.log(res.data[0]),
          setUltimoContrato(
            `${formatearFechaEspañol(
              res.data[0].CON_fecha_inn
            )} al ${formatearFechaEspañol(res.data[0].CON_fecha_out)}`
          ),
          setPenultimoContrato(
            `${formatearFechaEspañol(
              res.data[1].CON_fecha_inn
            )} al ${formatearFechaEspañol(res.data[1].CON_fecha_out)}`
          );
      })
      .catch((err) => {
        console.log(err);
  ...
  const getdate1 = async (e) => {
    ...
    setGetData((prevState) => ({
      ...prevState,
      fechaInicioContrato: fecha.value,
    }));
  ...
  return (
    <div className="RegistrarContrato">
        ...
        <div className="RegistrarContrato__cuerpo-contenido">
          <div className="RegistrarContrato__registrar">
            ...
            <div className="RegistrarContrato__fila4" id="fila4">
              ...
              <div className="RegistrarContrato__fila4" id="fila4">
                <h2>
                  {penultimoContrato === "" ? "" : "2) inicio - termino"}{" "}
                </h2>
          ...
          <div className="RegistrarContrato__tabla">
            <div className="RegistrarContrato__fila">
              <TextField
                ...
                name="horario"
                onChange={(e) => {
                  setDataHorario(e.target.value), capturarId(e.target.value);
                }}
              >
                {
                  (
      ...
      <Modal2
        ...
        ancho="400px"
      >
        <div style={{display: "grid", width: "100%", gap:"40px", height:"100%"}}>
          ...
          >
            <button
              type="button"
              onClick={() => (window.location.href = "/registrar%20personal")}
              className="button"
              style={{ width: "30%", height: "30px" }}
            >
              <h5>ir a registrar</h5>
            </button>
            <button
              type="button"
              className="button"
