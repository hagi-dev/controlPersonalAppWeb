...
const RegistrarTipo = () => {
  ...
  const enviarPost = async () => {
    try {
      ...
      if (editar === 0) {
        if (areas === 1 && cargos === 1) {
          await axios
            .post("/TipoTrabajador/registrar", getData)
            .then((res) => {
              getTipoTrabajador();
            })
            .catch((err) => {
        ...
        if (areas === 1 && cargos === 1) {
          await axios
            .put(`/TipoTrabajador/update/${ids}`, getData)
            .then((res) => {
  ...
  return (
    <div className="RegistrarTipo">
      ...
      <div
        className="RegistrarTipo__cuerpo"
      ...
      >
        ...
        <div className="RegistrarTipo__cuerpo-contenido">
          <div className="RegistrarTipo__registrar">
            <div className="RegistrarTipo__registrar-titulo" id="sudTitle">
              <h2>{subTitle}</h2>
            </div>
            <div className="RegistrarTipo__Combos">
              ...
              <div className="RegistrarTipo__fila">
                <TextField
                  ...
                  onChange={getSelection}
                >
                  {getData.area === "Almacen" ? (
                    cargosAlmacen.map((option1) => (
                      <MenuItem key={option1.id} value={option1.nombre}>
                        {option1.nombre}
                      </MenuItem>
                    ))
                  ) : getData.area === "Ventas" ? (
                    cargosVentas.map((option2) => (
                      <MenuItem key={option2.id} value={option2.nombre}>
                        {option2.nombre}
                      </MenuItem>
                    ))
                  ) : getData.area === "Marketing y diseño" ? (
          ...
          <div className="RegistrarTipo__lista" style={{ margin: "0" }}>
            ...
            >
              <MaterialTable
                columns={columns}
                data={data}
                title="lista de Tipo Trabajador"
                icons={tableIcons}
                  ...
                  },
                  {
                    ...
                    onClick: async (event, rowData) => {
                      if (
                        ...
                        console.log("este es id enviado", rowData.id);
                        await axios
                          .delete(`/TipoTrabajador/delete/${rowData.id}`)
                          .then((res) => {
                            getTipoTrabajador();
                          })
                          .catch((err) => {
